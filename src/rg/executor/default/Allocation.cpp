...
void Allocation::create_vk_resources() {
	const auto create_image = [&](const ImageBase *p_image) {
		...
		alloc.image.myvk_images[1] =
		    alloc.double_buffer ? std::make_shared<RGImage>(m_device_ptr, create_info) : alloc.image.myvk_images[0];
	};
	const auto create_buffer = [&](const BufferBase *p_buffer) {
		...
		alloc.buffer.myvk_buffers[1] =
		    alloc.double_buffer ? std::make_shared<RGBuffer>(m_device_ptr, create_info) : alloc.buffer.myvk_buffers[0];
	};
	...
	for (const ResourceBase *p_l : resources) {
		...
		for (const ResourceBase *p_r : resources) {
			...
			if (alloc_l.mem_offsets[0] + alloc_l.vk_mem_reqs.size > alloc_r.mem_offsets[0] &&
				...
				m_resource_alias_relation.Add(alloc_l.alloc_id, alloc_r.alloc_id);
		}
	}
...
void Allocation::create_vk_allocations(const Args &args) {
	m_resource_alias_relation.Reset(m_alloc_id_resources.size(), m_alloc_id_resources.size());
...
void Allocation::bind_vk_resources() {
	for (const ResourceBase *p_resource : m_alloc_id_resources) {
		auto &alloc = get_alloc_info(p_resource);
